        ...
        <article>
            ...
            <div id="content">
                ...
                if ($_SERVER['REQUEST_METHOD'] == "GET") {
                    $user = $_SESSION["login"];
                    $query = mysql_query("SELECT `user`,`subject`,`date`,`content`,`id` FROM `sendmessage` WHERE `from` = '" . $user . "'");
                    if ($query) {
                        if (mysql_num_rows($query) > 0) {
                            echo '<table><tr><td class="field">
                            <form action="removemessagesent.php" method="post">
                                <input type="hidden" value=' . $user . ' name="user">
                                <input type="submit" value="Usuń wiadomości">
                            </form></td></tr></table>';
                            while ($row = mysql_fetch_row($query)) {
                                echo '<table class="comment">
                                <tr><td class="field"><strong>Odbiorca:</strong></td><td> ' . $row[0] . '</td></tr>
                            ...
                            echo '<strong>Brak wiadomości w wysłanych!</strong>';
                        }
                    } else {
                        echo mysql_error();
